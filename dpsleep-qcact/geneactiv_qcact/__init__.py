...
def parse_args():
    argparser = ap.ArgumentParser('qcact Pipeline for GENEActiv')
...
def main(args):
    ...
    # logger output
    fh = logging.FileHandler(os.path.join(output_dir, 'qcact.log'))
...
def run_matlab(read_dir, output_dir, study, subject, date_from, mtl_dir):
    try:
        ...
        sub_cmd = "qcact('{READ}','{OUT}','{STUDY}','{SUBJECT}','{DATE_FROM}')"
...
def check_input(read_dir):
    if os.path.exists(read_dir):
        return os.path.join(read_dir, 'mtl5')
    else:
...
def check_output(output_dir):
    if os.path.exists(output_dir):
        output_dir = os.path.join(output_dir, 'mtl6')
        if os.path.exists(output_dir):
