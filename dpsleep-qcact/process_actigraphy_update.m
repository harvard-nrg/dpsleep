...
indrb=indr(1:id1);       indra=indr(id2:1440);
indbw=length(indwkb(indwkb==3));   indaw=length(indwka(indwka==3));
indbr=length(indwkb(indwkb==2 | indwkb==4));   indar=length(indwka(indwka==2 | indwka==4));
indbm=length(indrb(indrb>=15));   indam=length(indra(indra>15));
indbmp=100*length(indrb(indrb>=15))/length(indrb);   indamp=100*length(indra(indra>15))/length(indra);
%% Off-wrist minutes 6pm-6pm
ind6=indf;
indoff=length(ind6(ind6==0));
%% Inside Sleep Epoch
if ((id1==id2) || isempty(b1) || isempty(b2) || isempty(s1) || isempty(s2))    
    updm=[indoff,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,indbw,indaw,indbr,indar,indbm,indam,indbmp,indamp,0];
else    
    %% Bout detection
    ...
    %indbtf=movmean(indbt1,[0 0]);
    %indbtb=movmean(indbt1,[0 0]);
    %indbt(indbtf>.4 | indbtb>.4)=1;
    indbt=indbt1;
