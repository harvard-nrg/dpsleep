...
def parse_args():
    argparser = ap.ArgumentParser('act Pipeline for GENEActiv')
...
def main(args):
    ...
    # logger output
    fh = logging.FileHandler(os.path.join(output_dir, 'act.log'))
...
def check_input(read_dir):
    if os.path.exists(read_dir):
        return os.path.join(read_dir, 'mtl2')
    else:
...
def check_output(output_dir):
    if os.path.exists(output_dir):
        output_dir = os.path.join(output_dir, 'mtl3')
        if os.path.exists(output_dir):
